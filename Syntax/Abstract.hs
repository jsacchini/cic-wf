...
data Expr = Ann Pos Expr Expr -- annotated term and type
          ...
          | App Pos Expr Expr
          | Constr Pos CName IName Int [Param] [Arg]
          | Match Pos Expr Name IName [Name] Expr [Branch]
          | Fix Pos Int Name [Expr] Expr
          | Ind Pos Name 
--          deriving(Show)
instance Show (Expr) where
    show = show . tprint 0 []

type Branch = (Int, ([Name], Expr))

data Command = Definition Name (Maybe Expr) Expr
             | Axiom Name Expr
             | Load FilePath
             | Refine Name Expr Expr
             | Inductive IName [Param] [Arg] Sort [(CName, [Arg])]
...
tprint _ l (Bound _ n) = text (l !! n)
tprint _ _ (Var _ x) = text x
...
nestedLamDef (Lam _ x t1 t2) = ((varName,t1):ds, e)
