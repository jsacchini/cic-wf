...
module CICminus.TypeChecking.Permutation where
...
instance ApplyPerm A.FixExpr where
  applyPerm p (A.FixExpr rg k num nm stage tp body) =
    A.FixExpr rg k num nm stage (applyPerm p tp) (applyPerm (1 <++ p) body)
