...
{-|
  ...
  - Replace fakeBinds with CICminus.Utils.Fresh.withFresh
...
module CICminus.Syntax.InternalToAbstract where
...
-- import {-# SOURCE #-} CICminus.TypeChecking.PrettyTCM
...
instance Reify Term C.Expr where
  ...
  reify (Bound n) = do
    xs <- getLocalNames
    if n >= length xs
      then return $ C.Ident noRange (mkName ("ERROR "++show n)) C.LocalIdent
      else do
      v <- getVerbosity
      if v >= 30
        then return $ C.Ident noRange (mkName (show (xs !! n)
                                             ++ "["
                                             ++ show n ++ "]")) C.LocalIdent
        else return $ C.Ident noRange (mkName (show (xs !! n))) C.LocalIdent
  ...
  reify (Constr c _ pars) =
    ...
    then return $ C.Number noRange 0
      ...
      return $ C.mkApp (C.Ident noRange c C.ConstructorIdent) pars'
  ...
  reify (App t ts) =
    ...
    case (t, ts) of
      (Constr c0 _ [], [])
        ...
        | otherwise        -> return $ C.Ident noRange c0 C.ConstructorIdent
      ...
      (Constr c1 _ [], [t])
        ...
        | otherwise -> fmap (C.App noRange (C.Ident noRange c1 C.ConstructorIdent) explicitArg) (reify t)
      _ -> do
        e <- reify t
        es <- mapM reify ts
        return $ C.mkApp e es

  reify (Var x) = return $ C.Ident noRange x C.GlobalIdent

  reify (Ind annot ind pars) =
    liftM (C.mkApp ident) (mapM reify pars)
    where
      ident :: C.Expr
      ident =
        case annot of
          Empty -> C.Ident noRange ind C.CoInductiveIdent
          Star  -> C.SApp noRange ind C.CoInductiveIdent (C.SizeStar noRange)
          Stage s -> case s of
            I.Infty        -> C.SApp noRange ind C.CoInductiveIdent
                              (mkSizeExpr (mkName "∞") 0)
            I.StageVar v n -> C.SApp noRange ind C.CoInductiveIdent
                              (mkSizeExpr (mkName (show v)) n)
          SizeVar s n -> C.SApp noRange ind C.CoInductiveIdent
                         (mkSizeExpr s n)
...
instance Reify CaseTerm C.CaseExpr where
  reify (CaseTerm arg _ asName nmInd pars cin tpRet branches) = do
...
instance Reify Branch C.Branch where
  reify (Branch nmConstr idConstr patt body) = do
    ...
    body' <- pushCtx (renameCtx (ctxFromList patCtx) patt)
             $ reify body
    return $ C.Branch noRange nmConstr patt' body'
