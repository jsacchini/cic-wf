...
instance Scope C.Expr A.Expr where
  ...
  scope e@(C.SApp _ _ _ _) = scopeApp (e, [])
  scope (C.Meta r i) = return $ A.Meta r i
  scope e@(C.Ident _ _ _) = scopeApp (e, [])
...
instance Scope C.CaseExpr A.CaseExpr where
  scope (C.CaseExpr r arg asName indices ret bs) =
    do arg'     <- scope arg
       ...
       return $ A.CaseExpr r arg' asName indices' ret' sortbs
...
instance Scope C.Branch A.Branch where
  scope (C.Branch r constr pattern body) = do
            ...
            Just i@(I.Inductive {}) ->
                do
                  ...
                  if givenLen < parLen
                    ...
                    else return (A.mkApp (A.Ind iRange x A.SizeEmpty ipars) iargs)
                where
          ...
          do g <- lookupGlobal x
             case g of
               Just i@(I.Inductive {}) -> do
                  if givenLen < parLen
                    then inductiveNotApplied iRange x
                    else return (A.mkApp (A.Ind iRange x s' ipars) iargs)
                where
                  parLen = size (I.indPars i)
                  argLen = size (I.indIndices i)
                  givenLen = length args
