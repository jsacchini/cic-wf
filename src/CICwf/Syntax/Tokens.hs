...
module CICwf.Syntax.Tokens where
...
data Keyword = KwForall | KwFun | KwProp | KwType | KwLet | KwDefine | KwImport
             ...
             | KwEnd | KwWhere | KwOf | KwEval | KwCheck | KwCoIn | KwCocase
             | KwData | KwCodata
             | KwFix | KwCofix | KwRec
...
data Token = TokKeyword Keyword Position
           ...
           | TokLiftedIdent (Position, String)
...
identStar pos s = do tok <- ident pos (init s) -- remove the '*' at the end
                     case tok of
                       TokIdent (p',s') -> return $ TokIdentStar (p',s')
                       _                -> fail "Lexical error tokstar"

liftedIdent :: Position -> String -> Parser Token
liftedIdent pos s = do
  tok <- ident pos (tail s) -- remove the initial '!'
  case tok of
    TokIdent (p',s') -> return $ TokLiftedIdent (p',s')
    _                -> fail "Lexical error tokstar"
