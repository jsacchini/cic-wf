...
refine :: (MonadRM rm) => A.Expr -> rm (Type EVAR, Term EVAR, Subst)
refine (A.EVar _ _) = refinerError RefinerError
...
refine (A.Pi _ x t1 t2) = do (r1, t1', sigma1) <- refine t1
                             isSort r1
                             (r2, t2', sigma2) <- local ((x,t1'):) $ refine t2
                             isSort r2
...
check :: (MonadRM rm) => A.Expr -> Term EVAR -> rm (Term EVAR, Subst)
