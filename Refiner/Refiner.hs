...
refine_ (A.Pi _ x t1 t2) = do (r1, t1', sigma1) <- refine_ t1
                              s1 <- isSort r1
                              (r2, t2', sigma2) <- local (Bind x t1':) $ refine_ t2
                              s2 <- isSort r2
                              return (TSort $ checkProd s1 s2,
                                      Pi (Bind x (apply sigma2 t1')) t2',
                                      sigma1 <+> sigma2)
...
refine_ (A.Lam _ x t u) = do (r1, t', sigma1) <- refine_ t
                             isSort r1
                             (r2, u', sigma2) <- local (Bind x t':) $ refine_ u
                             -- s <- refine_ g l (Pi t r2)
                             -- isSort s
                             return (Pi (Bind x t') r2,
                                     Lam (Bind x (apply sigma2 t')) u',
                                     sigma1 <+> sigma2)
...
check t@(A.Lam _ x t1 t2) u = do (r1, _, _) <- refine_ (reify u) -- not nice
                                 (t1', sigmat) <- check t1 r1
                                 w1 <- whnf u
                                 case w1 of
                                   Pi u1 u2 -> do sigma1 <- unify t1' (expr u1)
                                                  (t2', sigma2) <- local (Bind x t1':) $ check t2 u2
                                                  let ss = sigmat <+> sigma1 <+> sigma2
                                                  return (Lam (Bind x (apply ss t1')) (apply ss t2'), ss)
